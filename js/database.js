// ============================================
// LEGALIA DATABASE - VERSIÓN COMPLETA
// ============================================
...
function iniciarBaseDatos(){return new Promise((res,rej)=>{let r=indexedDB.open(DB_NAME,DB_VERSION);
  ...
  r.onupgradeneeded=e=>{db=e.target.result;
    if(!db.objectStoreNames.contains('usuarios')){let s=db.createObjectStore('usuarios',{keyPath:'email'});s.createIndex('role','role');s.createIndex('nombre','nombre');}
    if(!db.objectStoreNames.contains('conversaciones')){let s=db.createObjectStore('conversaciones',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('fecha','fecha');}
    if(!db.objectStoreNames.contains('documentos')){let s=db.createObjectStore('documentos',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('estado','estado');s.createIndex('casoId','casoId');}
    if(!db.objectStoreNames.contains('casos')){let s=db.createObjectStore('casos',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('abogadoEmail','abogadoEmail');s.createIndex('estado','estado');}
    if(!db.objectStoreNames.contains('citas')){let s=db.createObjectStore('citas',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('casoId','casoId');s.createIndex('fecha','fecha');}
    if(!db.objectStoreNames.contains('pagos')){let s=db.createObjectStore('pagos',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('casoId','casoId');}
    if(!db.objectStoreNames.contains('calificaciones')){let s=db.createObjectStore('calificaciones',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('abogadoEmail','abogadoEmail');}
    if(!db.objectStoreNames.contains('notificaciones')){let s=db.createObjectStore('notificaciones',{keyPath:'id',autoIncrement:true});s.createIndex('usuarioEmail','usuarioEmail');s.createIndex('leida','leida');}
  };
});}
...
function autenticarUsuario(e,p,role){return new Promise((res,rej)=>{obtenerUsuarioPorEmail(e).then(u=>{if(!u)return rej('Usuario no encontrado');if(u.password!==p)return rej('Contraseña incorrecta');if(u.role!==role)return rej(`No eres ${role === 'administrador' ? 'administrador' : role}`);if(!u.activo)return rej('Cuenta desactivada');res(u);}).catch(rej);});}
...
function obtenerUsuariosPorRol(role){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');let req=db.transaction(['usuarios'],'readonly').objectStore('usuarios').index('role').getAll(role);req.onsuccess=()=>res(req.result||[]);req.onerror=e=>rej(e.target.error);});}
...
function agregarDocumento(ue,n,d,area,abogadoEmail=null){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');let req=db.transaction(['documentos'],'readwrite').objectStore('documentos').add({usuarioEmail:ue.toLowerCase(),nombre:n,descripcion:d,area:area,abogadoEmail:abogadoEmail,estado:'pendiente',archivo:null,fechaSolicitud:new Date().toISOString(),fechaSubida:null});req.onsuccess=()=>res(req.result);req.onerror=e=>rej(e.target.error);});}
...
function obtenerTodosLosCasos(){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');db.transaction(['casos'],'readonly').objectStore('casos').getAll().onsuccess=e=>res(e.target.result||[]);});}
function crearCaso(ue,un,n,t,d,a){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');let req=db.transaction(['casos'],'readwrite').objectStore('casos').add({usuarioEmail:ue.toLowerCase(),usuarioNombre:un,titulo:t,descripcion:d,area:a,estado:'Pendiente',abogadoEmail:null,abogadoNombre:null,fechaCreacion:new Date().toISOString(),ultimaActualizacion:new Date().toISOString()});req.onsuccess=()=>res(req.result);req.onerror=e=>rej(e.target.error);});}
function actualizarEstadoCaso(id,estado){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');db.transaction(['casos'],'readwrite').objectStore('casos').get(id).onsuccess=e=>{let c=e.target.result;if(c){c.estado=estado;c.ultimaActualizacion=new Date().toISOString();db.transaction(['casos'],'readwrite').objectStore('casos').put(c).onsuccess=()=>res(c);}else{rej('Caso no encontrado');}};});}}
function asignarAbogadoACaso(id,abogadoEmail,abogadoNombre){return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');db.transaction(['casos'],'readwrite').objectStore('casos').get(id).onsuccess=e=>{let c=e.target.result;if(c){c.abogadoEmail=abogadoEmail;c.abogadoNombre=abogadoNombre;c.estado='En Progreso';c.ultimaActualizacion=new Date().toISOString();db.transaction(['casos'],'readwrite').objectStore('casos').put(c).onsuccess=()=>res(c);}else{rej('Caso no encontrado');}};});}}
...
window.db={iniciar:iniciarBaseDatos,_db:null,
  usuarios:{crear:crearUsuario,obtener:obtenerUsuarioPorEmail,autenticar:autenticarUsuario,todos:obtenerTodosUsuarios,obtenerPorRol:obtenerUsuariosPorRol,actualizar:async(u)=>{return new Promise((res,rej)=>{if(!db)return rej('DB no inicializada');db.transaction(['usuarios'],'readwrite').objectStore('usuarios').put(u).onsuccess=()=>res(u);});}},
  ...
  casos:{obtenerPorUsuario:obtenerCasosUsuario,obtenerTodos:obtenerTodosLosCasos,crear:crearCaso,actualizarEstado:actualizarEstadoCaso,asignarAbogado:asignarAbogadoACaso},
